...
def iscircle(x, y, z, i, e):
    ...
    for j in range(m - 10):
        if abs(s_x[(i + j + 1) % m]) + abs(s_y[(i + j + 1) % m]) + abs(s_z[(i + j + 1) % m]) <= e:
            return True;
    return False
...
#	    sleep(0.2)
            ...
            if sys.argv[1] == 'drum':
                t = time()
                x, y, z = imu.read_accel()
                r = drum.play(yaw, pitch, roll, sqrt(x*x+y*y+z*z), t, p)
                if r is not None:
                    p = r
            elif sys.argv[1] == 'theremin':
                theremin.play(yaw, pitch, roll)
            elif sys.argv[1] == 'gesture':
                x, y, z = imu.read_accel()
#                print("%f, %f, %f" % (x, y, z))
                if iscircle(x, y, z, i % m, 2):
                     os.system('aplay /root/hack/samples/circle.wav &> /dev/null')
                     print('CIRCLEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE!!')
                i += 1
                if i % 100 == 0:
                     print(i)
